...
from transformers import __version__ as transformers_version, LlamaConfig
from transformers.models.llama.modeling_llama import (
    logger,
    BaseModelOutputWithPast,
    CausalLMOutputWithPast, LlamaPreTrainedModel,
)
...
#         print("Using LlamaForCausalLMPatched")
#         if config.hidden_size != 896:
#             self.model.upscale_layer = nn.Linear(896, config.hidden_size)
#
# MODEL_FOR_CAUSAL_LM_MAPPING.register(LlamaConfig, LlamaForCausalLMPatched, exist_ok=True)
...
def _fast_prepare_inputs_for_generation(self, input_ids, inputs_embeds=None, **kwargs,):
    ...
    if inputs_embeds is not None and kwargs["cache_position"][0] == 0: #or use past_key_values
        model_inputs = {"inputs_embeds": inputs_embeds, **kwargs, }
    else:
        model_inputs = {"input_ids": input_ids.contiguous(), **kwargs, }  # `contiguous()` needed for compilation use cases
...
def LlamaModel_fast_forward(
    ...
    *args, **kwargs,
    ...
    if inputs_embeds is None:
        ...
        if sum_mask is not None and desc_emb is not None and desc_emb.shape[-1] != 0:
            # if llama / if dim mismatch need to upscale desc_emb with a linear layer
            # Clone inputs_embeds to avoid in-place operations on a leaf tensor
            if desc_emb.shape[-1] != self.config.hidden_size:
                desc_emb = self.upscale_layer(desc_emb)
            assert torch.all(input_ids[sum_mask] == 151646)
            modified_inputs_embeds = inputs_embeds.clone()
            # Directly assign desc_emb to the positions in modified_inputs_embeds where sum_mask is True
            modified_inputs_embeds[sum_mask] = desc_emb.view(-1, desc_emb.size(-1))
    ...
    if attention_mask is None:
        padding_mask = None
    elif self.training and not self.config.use_custom_attn:
        attention_mask = None
        padding_mask = None
    else:
...
def _wrap_fast_inference(generate, device_type, dtype, model):
    ...
    def _fast_generate(*args, **kwargs):
        ...
        # For newer HF
        kwargs["cache_implementation"] = kwargs.pop("cache_implementation", "dynamic")
