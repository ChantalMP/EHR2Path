...
from patient_model.dataset import SectionIterableDataset, create_sample_to_section_dict_ed_adm
...
def save_embs(collected_embs, task=''):
    suffix = ""
    if task == "ed_admission" or task == "ed_icd":
        suffix = "_ed_stays"
    elif task == "los":
        suffix = "_icu_los"
    for key, value in collected_embs.items():
        stay_id, hour_idx = key
        with gzip.open(f'{mimiciv_path}all_summaries_last_int{suffix}/{stay_id}_{hour_idx}.pt.gz', 'wb') as f:
            torch.save(collected_embs[key], f)


def extract_embs_all(model, tokenizer, split, dataset_idx, task=''):
    model.eval()
    dataset = SectionIterableDataset(tokenizer=tokenizer, split=split, weighted_sampling=True, max_input_len=5000, max_output_len=None,
                                     ...
                                     add_gen_prompt_for_predict=False, dataset_idx=dataset_idx, summary_len=SUMMARY_LEN, task=task)
            ...
            for idx, extracted_summary_tokens in enumerate(last_hidden_states):
                ...
                if current_block_idx != block_idxs[idx]:
                    ...
                    if current_block_idx is not None:
                        executor.submit(save_embs, collected_embs, task=task)
...
def parse_args():
    ...
    parser.add_argument('--task', type=str, choices=["ed_admission", "ed_icd"], required=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_idx', type=int, required=True,
                        help="Index of the dataset to process (0, 1, 2, or 3)")
    args = parser.parse_args()
    dataset_idx = args.dataset_idx

    print("Creating sample to section dict")
    create_sample_to_section_dict_ed_adm(split="train", sections=sections, num_workers=1, task=task)
    create_sample_to_section_dict_ed_adm(split="val", sections=sections, num_workers=1, task=task)

    print("Extracting dataset idx ", dataset_idx)
    model, tokenizer = load_summ_model()

    extract_embs_all(model, tokenizer, "train", dataset_idx, task=task)
    # extract_embs_all(model, tokenizer, "val", dataset_idx)
    extract_embs_all(model, tokenizer, "val", dataset_idx=None, task=task)