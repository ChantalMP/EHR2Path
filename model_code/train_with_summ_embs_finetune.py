...
from patient_model.dataset import TextDataset, SummaryDataset, SummaryDatasetEmbs
# noinspection PyUnresolvedReferences
...
class SaveCallback(TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        # save weights of kwargs['model'].base_model.model.base_model.upscale_layer
        if hasattr(kwargs['model'].base_model.model.base_model, 'upscale_layer'):
            upscale_layer = kwargs['model'].base_model.model.base_model.upscale_layer
            upscale_layer_weights = upscale_layer.state_dict()
            torch.save(upscale_layer_weights, f"{args.output_dir}/checkpoint-{state.global_step}/upscale_layer_weights.pt")
...
def log_metrics(model, eval_dataloader, step, summary_len):
    ...
    for idx, batch in tqdm(enumerate(eval_dataloader)):
        ...
        if eval_dataloader.dataset.custom_attn_mask: #This code tests the model output if only the summary tokens are given as input
            ...
            inputs_part, last_tokens, attention_mask = inputs[:, :-CONV_TOKENS_AFTER_SUMMARY], inputs[:, -CONV_TOKENS_AFTER_SUMMARY:], attention_mask[:,:-CONV_TOKENS_AFTER_SUMMARY]
            with torch.no_grad():
            ...
            for layer_idx, layer in enumerate(summary_tokens):
                new_summary_tokens.append((summary_tokens[layer_idx][0][:,:,-SUMMARY_LENGTH:,:], summary_tokens[layer_idx][1][:,:,-SUMMARY_LENGTH:,:]))
            ...
            attention_mask = torch.cat([attention_mask, torch.tensor([1]*5).unsqueeze(dim=0).cuda()], axis=1)[:,-(SUMMARY_LENGTH+CONV_TOKENS_AFTER_SUMMARY):]
            ...
            generated_ids = model.generate(input_ids=inputs, past_key_values=summary_tokens, attention_mask=attention_mask, cache_implementation=None, position_ids=pos_id_curr, max_new_tokens=4000)

        else:
            generated_ids = model.generate(inputs, attention_mask=attention_mask, desc_emb=batch['desc_emb'].to(model.device), sum_mask=batch['sum_mask'].to(model.device), max_new_tokens=4000)
...
class CustomAttnDataCollatorForSeq2Seq:
    ...
    def __call__(self, features):
        ...
        padding_side = self.base_collator.tokenizer.padding_side if hasattr(self.base_collator, 'tokenizer') else self.base_collator.base_collator.tokenizer.padding_side
...
if __name__ == '__main__':
    ...
    val_split = 'val' #'test' if val_only else 'val'
    ...
    if config['model_config']['use_custom_attn'] or config['dataset_config']['dataset_name'] == 'mixed':
    ...
    custom_data_collator = CustomDataCollatorForSeq2Seq(base_collator, additional_keys=['desc_emb', 'sum_mask'])
    custom_data_collator_eval = CustomDataCollatorForSeq2Seq(base_collator, additional_keys=['desc_emb', 'sum_mask', 'stay_id', 'hour_idx'])
    custom_attn_collator = CustomAttnDataCollatorForSeq2Seq(base_collator, additional_keys=['attention_mask'])

    dataset_train = SummaryDatasetEmbs(tokenizer=tokenizer, split='train', max_input_len=config['dataset_config']['max_input_len'],
                                       max_output_len=config['dataset_config']['max_output_len'], model_name=config['training_config']['model_name'],
                                       sum_type=config['dataset_config']['sum_type'], merge_with_text=config['dataset_config']['merge_with_text'],
                                       summary_len=config['dataset_config']['summary_len'], used_text_hours=config['dataset_config']['used_text_hours'], drop_augment=config['dataset_config']['drop_augment'],
                                       summ_ratio=config['dataset_config']['summ_ratio'] if 'summ_ratio' in config['dataset_config'] else 0.33,
                                       task=config['dataset_config']['task'] if 'task' in config['dataset_config'] else "",
                                       direct_prediction_ft=config['dataset_config']['direct_prediction_ft'] if 'direct_prediction_ft' in config['dataset_config'] else False)

    dataset_val = SummaryDatasetEmbs(tokenizer=tokenizer, split='val', max_input_len=config['dataset_config']['max_input_len'],
                                     max_output_len=config['dataset_config']['max_output_len'], model_name=config['training_config']['model_name'],
                                     sum_type=config['dataset_config']['sum_type'], merge_with_text=config['dataset_config']['merge_with_text'],
                                     summary_len=config['dataset_config']['summary_len'], used_text_hours=config['dataset_config']['used_text_hours'], drop_augment=False, eval_mode=config['dataset_config']['eval_mode'],
                                     task=config['dataset_config']['task'] if 'task' in config['dataset_config'] else "",
                                     direct_prediction_ft=config['dataset_config']['direct_prediction_ft'] if 'direct_prediction_ft' in config['dataset_config'] else False)

    dataset_callback_eval = SummaryDatasetEmbs(tokenizer=tokenizer, split=val_split, max_input_len=config['dataset_config']['max_input_len'],
                                               max_output_len=config['dataset_config']['max_output_len'], predict=True, model_name=config['training_config']['model_name'],
                                               sum_type=config['dataset_config']['sum_type'], merge_with_text=config['dataset_config']['merge_with_text'],
                                               summary_len=config['dataset_config']['summary_len'], used_text_hours=config['dataset_config']['used_text_hours'], drop_augment=False, eval_mode=config['dataset_config']['eval_mode'],
                                               task=config['dataset_config']['task'] if 'task' in config['dataset_config'] else "",
                                               direct_prediction_ft=config['dataset_config']['direct_prediction_ft'] if 'direct_prediction_ft' in config['dataset_config'] else False)
    ...
    eval_dataloader_callback = DataLoader(
        ...
        batch_size=8,
        collate_fn=custom_data_collator_eval,
        num_workers=1,
        pin_memory=True,
    )

    # load curriculum checkpoint of pre-trained summ-only model
    # if resume_from_checkpoint instead continue checkpoint from there (should be ckpt saved during curriculum training)
    if config['training_config']['resume_from_model'] and not config['training_config']['resume_from_checkpoint']:
        print("Starting from curriculum or fine-tuning checkpoint")
        curriculum_checkpoint = f"{config['training_config']['resume_from_model']}/adapter_model.safetensors"
        # load curriculum checkpoint
        lora_weights = load_file(curriculum_checkpoint)
    ...
    trainer = SFTTrainer_US(
        ...
        callbacks=[#EvalCallback(eval_dataloader_callback=eval_dataloader_callback, frequency=config['training_config']['eval_steps']*config['training_config']['eval_gen_frequency'], summary_len=config['dataset_config']['summary_len']),
                   ClearMemoryCallback(), SaveCallback()]
    )
    ...
    if not val_only:
        ...
        lora_weights_adapted.update({k: v for k, v in lora_weights.items() if 'lora' not in k})
        model.load_state_dict(lora_weights_adapted, strict=False)
        if os.path.exists(f"{config['training_config']['resume_from_checkpoint']}/upscale_layer_weights.pt"):
            upscale_layer_weights = torch.load(f"{config['training_config']['resume_from_checkpoint']}/upscale_layer_weights.pt")
            model.base_model.model.model.upscale_layer.load_state_dict(upscale_layer_weights)
        # trainer.evaluate()
        log_metrics(model, eval_dataloader_callback, 0, summary_len=8)
